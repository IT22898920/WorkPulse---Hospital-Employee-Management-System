...
import { Calendar, Plus, Edit, Trash2, Users, Clock, MapPin, Filter, Search, Download, ChevronLeft, ChevronRight } from 'lucide-react';
import { getAllSchedules, getSchedulesByDateRange, getScheduleStats, createSchedule, updateSchedule, cancelSchedule } from '../../services/scheduleService';
import employeeService from '../../services/employeeService';

const ShiftManagement = () => {
  const [currentView, setCurrentView] = useState('calendar'); // 'calendar' or 'list'
  const [selectedDate, setSelectedDate] = useState(new Date(2025, 9, 21)); // Start with October 21 2025 where schedules exist
  ...
  const filteredSchedules = schedules.filter(schedule => {
    const matchesSearch = !searchTerm ||
      `${schedule.employee?.firstName} ${schedule.employee?.lastName}`.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const fetchSchedules = async () => {
    try {
      ...
      console.log('📅 Fetching schedules for date range:', {
        ...
        currentlyViewingMonth: selectedDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
        selectedDateDetails: {
      ...
      if (response.data?.length > 0) {
        console.log('📋 First 3 schedules with dates:', response.data.slice(0, 3).map(s => {
      ...
      const schedulesInCurrentMonth = response.data?.filter(schedule => {
        const scheduleDate = new Date(schedule.date);
        return scheduleDate.getMonth() === selectedDate.getMonth() &&
               scheduleDate.getFullYear() === selectedDate.getFullYear();
      }) || [];

      console.log('📅 Schedules in current month view:', schedulesInCurrentMonth.length);
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const employeeList = response.data?.filter(emp => emp.role === 'employee') || [];
  ...
  const deleteSchedule = async (scheduleId) => {
    if (!window.confirm('Are you sure you want to permanently delete this shift? This action cannot be undone.')) return;
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2 text-lg">Create, view, and manage employee shifts with ease</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-6 text-white shadow-xl">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold">{stats.overall?.totalSchedules || 0}</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl p-6 text-white shadow-xl">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold">{stats.overall?.scheduledCount || 0}</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white shadow-xl">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold">{stats.overall?.completedCount || 0}</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-6 text-white shadow-xl">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold">{stats.overall?.nightShifts || 0}</p>
                </div>
            ...
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    onClick={() => {
                      console.log('🎯 Navigating to October 21, 2025 to view schedules');
            ...
            <div className="p-6">
              ...
              {filterDateRange === 'all' ? (
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            <div className={`w-4 h-4 rounded-full ${
                              schedule.shift?.type === 'morning' ? 'bg-yellow-400' :
                              schedule.shift?.type === 'afternoon' ? 'bg-orange-400' :
                              schedule.shift?.type === 'night' ? 'bg-purple-400' : 'bg-gray-400'
                            }`}></div>
                            <h4 className="font-semibold text-gray-900">
                              {schedule.employee?.firstName} {schedule.employee?.lastName}
                            </h4>
                            <span className="text-sm text-gray-500">({schedule.employee?.employeeId})</span>
                          </div>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                              <span className="text-gray-500">Date:</span>
                              <div className="font-medium">{new Date(schedule.date).toDateString()}</div>
                            </div>
                            <div>
                              <span className="text-gray-500">Time:</span>
                              <div className="font-medium">{schedule.shift?.startTime} - {schedule.shift?.endTime}</div>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <div className={`font-medium ${schedule.status === 'completed' ? 'text-green-600' : 'text-blue-600'}`}>
                                {schedule.status}
                              </div>
                            </div>
                          </div>
                ...
                {calendarDays.map((day, index) => {
                  ...
                  if (index < 3) {
                    console.log(`📅 Day ${day.getDate()}: Found ${daySchedules.length} schedules`);
                  }
                  ...
                  return (
                    ...
                    >
                      <div className={`font-semibold text-sm mb-2 ${isToday ? 'text-blue-700' : ''}`}>
                        {day.getDate()}
                      </div>
                            ...
                            >
                              ...
                              >
                                <div className="font-semibold truncate text-xs leading-tight">
                                  {schedule.employee?.firstName.charAt(0)}.{schedule.employee?.lastName}
                                </div>
            ...
            <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-purple-50">
              <div className="flex justify-between items-center">
                ...
                <div className="text-sm text-gray-600">
                  Showing <span className="font-semibold text-blue-600">{filteredSchedules.length}</span> of <span className="font-semibold">{schedules.length}</span> shifts
                </div>
              </div>
            </div>
              ...
              <table className="min-w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {filteredSchedules.map((schedule) => (
                    <tr key={schedule._id} className="hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-200">
...
const CreateEditShiftModal = ({ schedule, employees, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Error saving schedule: ' + (error.response?.data?.message || error.message));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-screen overflow-y-auto m-4">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 rounded-t-2xl">
          ...
          <p className="text-blue-100 mt-1">
            {schedule ? 'Update shift details and schedule' : 'Schedule a new shift for your employee'}
          </p>
        </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({
                      ...formData,
                      location: { ...formData.location, unit: e.target.value }
                    })}
                    className="w-full rounded-xl border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 transition-colors p-3"
                  />
                </div>
            ...
            <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
              ...
              >
                {loading ? (
                  ...
                  schedule ? '✏️ Update Shift' : '➕ Create Shift'
                )}
              </button>
            </div>
          </form>
